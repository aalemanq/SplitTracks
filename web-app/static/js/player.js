class Player {
  ...
  async load(stemsData) {
    ...
    for (const s of stemsData) {
      try {
        ...
        console.warn('Error loading stem:', s.name, e);
      }
    }
  ...
  _updateGains() {
    ...
    for (const g of this.gains) {
      const stem = this.stems.find(s => s.name === g.name);
      if (!stem) continue;
      let audible = !stem.mute;
      if (hasSolo) audible = audible && stem.solo;
  ...
  _stopSources() {
    for (const src of this.sources) {
      try { src.stop(); } catch (e) { /* ya parado */ }
    }
    this.sources = [];
  }
  ...
  play() {
    ...
    this._createSources(this.pausedAt);
  ...
  setVolume(index, vol) {
    if (index < this.stems.length) {
      this.stems[index].volume = Math.max(0, Math.min(1.5, vol));
      this._updateGains();
    }
  }
